...
const loadedReducer = (state, action) => {
  switch(action) {
    ...
    case 'IS_LOADED': return false;
    default: return state;
  }
}

const Reducers = combineReducers({Todos: todoReducer, Books: bookReducer, Loaded: loadedReducer});

function App () {
  const AppContext = context;
  const [state, dispatch] = useReducer(Reducers, {Books: [], Todos: [], Loaded: false});
